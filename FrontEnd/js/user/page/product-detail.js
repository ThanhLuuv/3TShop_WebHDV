...
const productDetailSpinner = document.querySelector('#product-detail-spinner');
const sameProductSpinner = document.querySelector('#same-product-spinner');
const imageContainer = document.querySelector('.detail-container .image');
...
function showProductDetailLoading() {
    productDetailSpinner.style.display = 'block';
    imageContainer.classList.add('loading');
    productDetailContainer.classList.add('loading');
}

function hideProductDetailLoading() {
    productDetailSpinner.style.display = 'none';
    imageContainer.classList.remove('loading');
    productDetailContainer.classList.remove('loading');
}

function showSameProductLoading() {
    sameProductSpinner.style.display = 'block';
    sameProductList.classList.add('loading');
}

function hideSameProductLoading() {
    sameProductSpinner.style.display = 'none';
    sameProductList.classList.remove('loading');
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function fillData(data) {
        ...
        productDetailContainer.innerHTML = `
            <div class="navigate">
                <a href="/">Trang chủ / </a>
                <a href="/category/${data.categoryDTO.category_id}">${data.categoryDTO.name} / </a>
                <a href="">${data.name}</a>
            </div>
            <div>
    ...
    function setUpNextPrev() {
        ...
        const nextBtn = document.querySelector(".next-same-product");
        const prevBtn = document.querySelector(".prev-same-product");

        totalSameProduct > state.itemsPerPageSameProduct ? nextBtn.style.display = "inline-block" : nextBtn.style.display = "none";
        prevBtn.style.display = "none";

        function updateCarousel() {
            const offset = -(state.currentIndexSameProduct * 25);
            const pixel = state.currentIndexSameProduct * 14;
            sameProductList.style.transform = `translateX(calc(${offset}% - ${pixel}px))`;
        }

        function prevSameProduct() {
            if (state.currentIndexSameProduct > 0) {
                state.currentIndexSameProduct--;
                updateCarousel();
                if (state.currentIndexSameProduct <= 0) {
                    prevBtn.style.display = "none";
                }
        ...
        function nextSameProduct() {
            if (state.currentIndexSameProduct < totalSameProduct - state.itemsPerPageSameProduct) {
                state.currentIndexSameProduct++;
                updateCarousel();
                if (state.currentIndexSameProduct >= totalSameProduct - state.itemsPerPageSameProduct) {
        ...
        prevBtn.addEventListener("click", prevSameProduct);
        nextBtn.addEventListener("click", nextSameProduct);
    }
    ...
    async function fetchData(id) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                await fillDataAsync(state.data);
